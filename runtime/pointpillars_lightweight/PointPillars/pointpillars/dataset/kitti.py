...
class Kitti(Dataset):
    ...
    def __init__(self, data_root, split, pts_prefix='velodyne_reduced'):
        ...
        # --- FIX: Filter out frames with NO Cars INSIDE RANGE ---
        valid_ids = []
        for id in self.sorted_ids:
            annos = self.data_infos[id]['annos']
            locs = annos['location']  # (N, 3) Camera Coordinates
            names = annos['name']  # (N,)
            ...
            for i, name in enumerate(names):
                if name == 'Car':
                    ...
                    # PointPillars LiDAR range (forward) is [0, 69.12]
                    # This corresponds to Camera Z.
                    z_depth = locs[i][2]

                    # Check if car is within the forward range the model expects
                    if z_depth > 0 and z_depth < 70:
                        has_valid_car = True
                        break
        ...
        print(
            f"Dataset split '{split}': Loaded {len(self.sorted_ids)} frames. Kept {len(valid_ids)} frames with valid Cars.")
        self.sorted_ids = valid_ids
        # -------------------------------------------
    ...
    def __getitem__(self, index):
        ...
        # --- FINAL FIX: RESAMPLING ---
        # If augmentation pushed all cars out of bounds, data_dict['gt_labels'] will be empty.
        # This causes the crash in anchor generation.
        # We recursively call __getitem__ with a random index to get a valid frame.
        if len(data_dict['gt_labels']) == 0:
